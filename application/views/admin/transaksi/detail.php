...
<div class="container-fluid">
    ...
    <p class="pull-right">
            <div class="btn-group pull-right">
                <a href="<?php echo base_url('penjualan/transaksi') ?>" title="Kembali" class="btn btn-info btn-sm m-1">
                    <i class="fa fa-backward"></i> Kembali
                </a>
                <?php if($header_transaksi->status_bayar == "Belum Bayar" || $header_transaksi->status_bayar == "Menunggu Verifikasi"){
                    echo '<a href="#" class="btn btn-success btn-sm m-1 disabled"> <i class="fa fa-print"></i> Cetak </a>';
                }else{ ?>
                    <a href="<?php echo base_url('penjualan/transaksi/cetak/'.$header_transaksi->kode_transaksi) ?>" target="_blank" title="Cetak" class="btn btn-success btn-sm m-1">
                    <i class="fa fa-print"></i> Cetak
                    </a>
                <?php } ?>

                <?php if($header_transaksi->status_bayar == "Belum Bayar" || $header_transaksi->status_bayar == "Sudah Bayar"){
                    echo '<a href="#" class="btn btn-warning btn-sm m-1 disabled"> <i class="fa fa-check"></i> Verifikasi Pembayaran </a>';
                }else{ ?>
                    <a href="<?php echo base_url('transaksi/verifikasi/'.$header_transaksi->kode_transaksi) ?>" class="btn btn-warning btn-sm m-1" onclick="return confirm('Pastikan pembayaran sudah benar. Verifikasi pembayaran pelanggan?')">
